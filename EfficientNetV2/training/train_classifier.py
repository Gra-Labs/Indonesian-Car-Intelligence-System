# train_classifier_augmented_v2.py - Versi perbaikan dengan hyperparameter optimal dan EfficientNetV2-M
import torch
import torch.nn as nn
import torch.optim as optim
from torch.optim import lr_scheduler
from torchvision import datasets, models, transforms
from torchvision.models import EfficientNet_V2_M_Weights # <-- Impor bobot EfficientNetV2-M
...
MODEL_SAVE_PATH = 'models/best_classifier_augmented_v2_efficientnetv2m.pt' # <-- Nama file model baru
...
# --- GANTI FUNGSI create_resnet50_classifier_v2 dengan EfficientNetV2 ---
def create_efficientnetv2_m_classifier(num_classes):
    """Model EfficientNetV2-M untuk klasifikasi mobil"""
    # Muat EfficientNetV2-M pre-trained pada ImageNet
    ...
    # EfficientNetV2 memiliki struktur yang sedikit berbeda untuk pembekuan
    # Biasanya, kita membekukan semua 'features' (badan utama model)
    for param in model_ft.features.parameters():
        param.requires_grad = False
    
    # Unfreeze lapisan terakhir dari features (misal, bagian 'blocks[-1]' jika ada)
    # Ini opsional, bisa juga membekukan semua features dan hanya melatih head
    # For EfficientNetV2, 'features' is a Sequential module
    # for param in model_ft.features[-1].parameters():
    #     param.requires_grad = True
    
    # Classifier head baru
    # Ambil in_features dari lapisan Linear terakhir di classifier bawaan EfficientNetV2
    num_ftrs = model_ft.classifier[-1].in_features
    
    model_ft.classifier = nn.Sequential(
        nn.Dropout(0.3), # Dropout pertama (sesuai dengan dropout_rate Anda)
    ...
    print(f"EfficientNetV2-M model created with {num_classes} classes")
...
if __name__ == '__main__':
    ...
    model_ft = create_efficientnetv2_m_classifier(NUM_CLASSES) # <-- Panggil fungsi EfficientNetV2
    ...
    plt.tight_layout()
    plt.savefig('training_performance_augmented_v2_efficientnetv2m.png', dpi=300, bbox_inches='tight') # <-- Nama file plot baru
    plt.show()
