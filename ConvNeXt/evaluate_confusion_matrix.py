...
# Konfigurasi
MODEL_PATH = 'models/best_classifier_augmented_v2_convnext_base.pt'
...
def main():
    ...
    try:
        ...
        # Try using the alternative loading function first
        from models.car_classifier import load_trained_convnext_classifier
        model = load_trained_convnext_classifier(MODEL_PATH, num_classes=NUM_CLASSES, device=device)
        print("Model loaded successfully using direct ConvNeXt loader.")
    except Exception as e:
        ...
        try:
            ...
            # Map keys from saved model (no prefix) to CarClassifier (with 'model.' prefix)
            mapped_state_dict = {}
            for key, value in saved_state_dict.items():
                # Add 'model.' prefix to all keys to match CarClassifier structure
                new_key = f"model.{key}"
    ...
    output_file = 'confusion_matrix_convnext.png'
